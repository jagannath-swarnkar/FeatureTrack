...
axios.interceptors.response.use(
	...
	async (error) => {
		...
		} else if (error.response.status === 401 && error.response?.data?.status === 401) {
			clearSession();
 ...
 * @return {*}
 */
export const getApi = (endpoint: string, otherHeaders: object = {}): Promise<IApiOutput<any>> => {
...
export const getWithToken = (endpoint: string, otherHeaders: object = {}): Promise<any> => {
	axios.defaults.headers.common["authorization"] = ParseToken(getCookie("token") || basicAuth);
	return axios.get(getUrl(endpoint), {
		headers: { ...commonHeader, ...otherHeaders }
	});
};
...
export const patchWithToken = (endpoint: string, data: any, otherHeaders: object = {}): Promise<any> => {
	axios.defaults.headers.common["authorization"] = ParseToken(getCookie("token"));
	return axios.patch(getUrl(endpoint), data, {
		headers: { ...commonHeader, ...otherHeaders }
	});
};
